package org.firstinspires.ftc.teamcode.Utils;
@com.acmerobotics.dashboard.config.Config
public class Config {
    public static double TRACKWIDTH = 1;
    public static double TICKS_TO_CM = 0.00268446637;
    public static double WHEEL_DIAMETER = 3.5;
    public static int CENTER_WHEEL_OFFSET = 0;
    public static int TIMP_CM = 50;

    public static double DESCHIDERE_CLESTE = 0.4;

    public static double kAIntake = 180;

    public static double OUTTAKE_CLOSEDPOS = 0.0;
    public static double OUTTAKE_OPENPOS = 0.3;

    public static double targetR = 15;

    public static double targetT = 0.5;

    public static double hV = 0.8, hA = 1, hBr = 15;
    public static double vV = 0.8, vA = 1, vBr = 15;
    public static double rV = 0.6, rA = 1, rBr = 20;

    public static double toleranceXY = 2.5;
    public static double toleranceR = 3;

    public static double minimumRate = 1.5;
}